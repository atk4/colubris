...
class ApiTimeAllRightsTest extends PHPUnit_Framework_TestCase {
    ...
    public function testCreateTask(
        ...
        // obj :: data :: id
        $this->assertObjectHasAttribute('id',$m,'Saved Task doesn\'t have ID');
        $this->assertTrue(!is_null($m->id),'Saved Task doesn\'t have ID');
    ...
    public function testCreateTime(
        App_CLI $app, Model_User $user, Model_User_Right $rights, $login_res_success, Model_Project $project,
        Model_Quote $quote, Model_Requirement $requirement, Model_Task $task
    ) {
        $this->app = $app;

        $url = 'v1/time/saveParams&lhash='.$login_res_success->hash->lhash;
        $data = [
            'task_id' => $task->id,
            'user_id' => $user->id,
            'spent_time' => 1
    ...
    public function testGetTime(
        ...
        // obj :: result
        $this->assertObjectHasAttribute('result',$obj,'No result is returned form API after getting a Time');
        $this->assertTrue(is_string($obj->result),'Time. Result was converted not to string by json_encode()');
        $this->assertEquals($obj->result,'success','Result of getting a Time is not successful');

        // obj :: data
        $this->assertObjectHasAttribute('data',$obj,'No data is returned form API after getting a Time');
        $this->assertTrue(is_array($obj->data),'Data is not an array after convertation of API respond on getting a Time');

        // obj :: data[0]
        $this->assertTrue(isset($obj->data[0]),'Data do not contain Time');
        $this->assertTrue( (count($obj->data)==1),'There is more then one Time in API respond on getting a Time by ID');
        $this->assertTrue(is_a($obj->data[0],'stdClass'),'Data[0] is not an object of class stdClass after convertation of API respond on getting a Time by ID');
